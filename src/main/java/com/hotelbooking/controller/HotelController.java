...
public class HotelController {
     ...
     * 创建酒店
     */
    public ApiResponse<Object> createHotel(String name, String location, String description,String amenities, Integer availableRooms) {
        try {
            Hotel hotel = hotelService.createHotel(name, location, description, amenities, availableRooms);
   ...
   public ApiResponse<Object> getHotelById(Integer hotelId) {
    ...
    public ApiResponse<Object> searchHotels(String name, String location) {
        try {
            ...
            // Normalize input
            String n = (name == null || name.trim().isEmpty()) ? null : name.trim().toLowerCase();
            String loc = (location == null || location.trim().isEmpty()) ? null : location.trim().toLowerCase();

            List<Hotel> filteredHotels = allHotels.stream()
                .filter(hotel -> {
                    String hotelName = hotel.getName().toLowerCase();
                    String hotelLoc = hotel.getLocation().toLowerCase();

                    boolean matchName = (n == null) || hotelName.contains(n);
                    boolean matchLoc = (loc == null) || hotelLoc.contains(loc);
